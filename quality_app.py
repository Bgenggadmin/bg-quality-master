...
stages = get_dynamic_list(df, "Stage", ["RM Inspection", "Marking", "Fit-up", "Welding", "Final"])
...
with st.form("quality_form", clear_on_submit=True):
    ...
    with col1:
        ...
        job_code = st.text_input("Type Job Code") if j_sel == "➕ Add New" else j_sel
        
        # 2. INSPECTOR
        i_sel = st.selectbox("Inspector", ["-- Select --", "➕ Add New"] + inspectors)
        inspector = st.text_input("Type Inspector Name") if i_sel == "➕ Add New" else i_sel
        
    with col2:
        # 3. STAGE
        s_sel = st.selectbox("Stage", ["-- Select --", "➕ Add New"] + stages)
        stage = st.text_input("Type Stage Name") if s_sel == "➕ Add New" else s_sel
    ...
    if st.form_submit_button("🚀 Submit & Sync"):
        # Check if they actually typed something when 'Add New' was picked
        if any(v in ["-- Select --", ""] for v in [job_code, inspector, stage]):
            st.error("❌ Fill all fields. If you picked 'Add New', you must type the name.")
        else:
            img_str = ""
            if cam_photo:
